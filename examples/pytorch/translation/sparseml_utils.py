...
class SparseMLSeq2SeqTrainer(SparseMLTrainer):
    """
    Seq2Seq trainer with SparseML integration
...
class Seq2SeqModuleExporter(ModuleExporter):
    """
    Module exporter class for Seq2Seq
    """

    @classmethod
    def get_output_names(self, out: Any):
        if not isinstance(out, Seq2SeqModelOutput):
            raise ValueError("Expected Seq2SeqModelOutput, got {type(out)}")
