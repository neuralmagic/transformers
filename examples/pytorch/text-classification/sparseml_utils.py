            ...
            if os.path.isdir(self.model_name_or_path):
                if os.path.isfile(os.path.join(self.model_name_or_path, WEIGHTS_NAME)):
                    ...
                    if new_params_to_init:
                        ...
                        _, missing_keys, unexpected_keys, _ = BertForSequenceClassification._load_state_dict_into_model(
...
def export_model(model, dataloader, output_dir, num_exported_samples):
    ...
    """
    exporter = GLUEModuleExporter(model, output_dir=output_dir)
    ...
    forward_args_spec = inspect.getfullargspec(BertForSequenceClassification.forward)
